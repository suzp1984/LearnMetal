...
typedef struct FragmentMaterialArguments {
    ...
    texture2d<half> emissionTexture [[ id(FragmentArgumentMaterialBufferIDEmission) ]];
                               ...
                               constant float3 &viewPos [[buffer(FragmentInputIndexViewPos)]])
{
    ...
    // emission
    float3 emission = float3(material.emissionTexture.sample(textureSampler, in.texCoords).rgb);
    
    float3 result = ambient + diffuse + specular + emission;
