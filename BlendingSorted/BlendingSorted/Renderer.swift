...
class Renderer: NSObject {
    ...
    init(metalView: MTKView) {
        ...
        windowTexture = try! Texture.newTextureWithName("window",
                                                       scaleFactor: 1.0,
                                                       device: device,
                                                       index: Int(FragmentInputIndexDiffuseTexture.rawValue))
    ...
    func handleCameraEvent(deltaX: Float, deltaY: Float) -> Void {
        ...
        windowUnifroms.viewMatrix = camera.getViewMatrix()
    }
...
extension Renderer : MTKViewDelegate
{
    func mtkView(_ view: MTKView, drawableSizeWillChange size: CGSize) {
        ...
        windowUnifroms.projectionMatrix   = projectionMatrix
    }
    ...
    func draw(in view: MTKView) {
        ...
        // sort windows
        let sortedWindows = windowsPositions.sorted(by: { a, b in
            return length(camera.cameraPosition - a) > length(camera.cameraPosition - b)
        })
        ...
        // draw windows
        try! renderEncoder.setFragmentTexture(windowTexture)
        
        for i in 0..<sortedWindows.count {
            let position = sortedWindows[i]
            
            windowUnifroms.modelMatrix = simd_mul(
                matrix4x4_translation(position),
                matrix4x4_rotation(Float.pi / 2.0, 1.0, 0.0, 0.0))
            renderEncoder.setVertexBytes(&windowUnifroms, length: MemoryLayout<Uniforms>.stride, index: Int(VertexInputIndexUniforms.rawValue))
