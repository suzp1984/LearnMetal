...
typedef struct PBRMaterial {
    texture2d<half> albedoMap [[id(MaterialArgumentIndexAlbedo)]];
    texture2d<half> normalMap [[id(MaterialArgumentIndexNormal)]];
    texture2d<half> metallicMap [[id(MaterialArgumentIndexMetallic)]];
    texture2d<half> roughnessMap [[id(MaterialArgumentIndexRoughness)]];
    texture2d<half> aoMap [[id(MaterialArgumentIndexAO)]];
} PBRMaterial;

fragment float4 pbrFragmentShader(RasterizerData in [[stage_in]],
                                  constant PBRMaterial &material [[buffer(FragmentInputIndexMaterial)]],
...
{
    // materail properties
    float3 albedo = pow(float3(material.albedoMap.sample(Const::linearSampler, in.texCoords).rgb), float3(2.2));
    float metallic = material.metallicMap.sample(Const::linearSampler, in.texCoords).r;
    float roughness = material.roughnessMap.sample(Const::linearSampler, in.texCoords).r;
    float ao = material.aoMap.sample(Const::linearSampler, in.texCoords).r;
...
//        float3 F = fresnelSchlick(clamp(dot(H, V), 0.0, 1.0), F0);
    ...
    float2 brdf = float2(pbrArguments.brdfMap.sample(Const::linearSampler, float2(max(dot(N, V), 0.0), roughness)).rg);
