...
typedef struct FragmentLightArguments {
    Light  light   [[id(FragmentArgumentLightBufferIDLight)]];
                               ...
                               device FragmentLightArguments &lightBuffer [[buffer(FragmentInputIndexLight)]])
{
    ...
    // ambient
    float3 ambient = lightBuffer.light.ambient * float3(material.diffuseTexture.sample(textureSampler, in.texCoords).rgb);
    
    // diffuse
    float3 norm = normalize(in.normal);
    float3 lightDir = normalize(lightBuffer.light.position - in.fragPos);
    float diff = max(dot(norm, lightDir), 0.0);
    float3 diffuse = lightBuffer.light.diffuse * (diff * float3(material.diffuseTexture.sample(textureSampler, in.texCoords).rgb));
    
    // specular
    float3 viewDir = normalize(lightBuffer.viewPos - in.fragPos);
    float3 reflectDir = reflect(-lightDir, norm);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
    float3 specular = lightBuffer.light.specular * (spec * float3(material.specularTexture.sample(textureSampler, in.texCoords).rgb));
    
    // attenuation
    float distance = length(lightBuffer.light.position - in.fragPos);
    float attenuation = 1.0 / (lightBuffer.light.constants + lightBuffer.light.linear * distance + lightBuffer.light.quadratic * (distance * distance));
