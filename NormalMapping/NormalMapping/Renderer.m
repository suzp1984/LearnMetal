...
@implementation Renderer
{
    ...
    id<MTLTexture> _brickwall;
    id<MTLTexture> _brickwallNormal;
...
- (nonnull instancetype)initWithMetalKitView:(nonnull MTKView*)mtkView {
    ...
    if (self) {
        ...
        _brickwall = [textureLoader newTextureWithName:@"brickwall"
                                      scaleFactor:1.0
                                           bundle:bundle
                                          options:@{ MTKTextureLoaderOptionSRGB: [NSNumber numberWithBool:NO] }
                                            error:&error];
        NSAssert(_brickwall, @"can not load brickwall texutre: %@", error);
        
        _brickwallNormal = [textureLoader newTextureWithName:@"brickwall_normal"
                                                 scaleFactor:1.0
                                                      bundle:bundle
                                                     options:nil
                                                       error:&error];
        NSAssert(_brickwallNormal, @"can not load brickwall normal texture: %@", error);
...
- (void)drawInMTKView:(nonnull MTKView *)view {
    ...
    [renderEncoder setFragmentTexture:_brickwall atIndex:FragmentInputIndexDiffuseMap];
    [renderEncoder setFragmentTexture:_brickwallNormal atIndex:FragmentInputIndexNormalMap];
