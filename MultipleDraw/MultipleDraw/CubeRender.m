...
//                                                  matrix4x4_scale(_size.x, _size.y, _size.z));
        ...
        _indirectCommandBuffer = [device newIndirectCommandBufferWithDescriptor:icbDescriptor maxCommandCount:cubeNum options:MTLResourceStorageModeShared];
        
        for (int i = 0 ; i < cubeNum; i++) {
            id<MTLIndirectRenderCommand> icbRenderCommand = [_indirectCommandBuffer indirectRenderCommandAtIndex:i];
            [icbRenderCommand setVertexBuffer:_verticesBuffer offset:0 atIndex:VertexInputIndexVertices];
            [icbRenderCommand setVertexBuffer:_uniformBuffer offset:0 atIndex:VertexInputIndexUniforms];
            [icbRenderCommand setVertexBuffer:_indirectObjectParamsBuffer offset:0 atIndex:VertexInputIndexObjParams];
            [icbRenderCommand setFragmentBuffer:_fragmentArgumentBuffer offset:0 atIndex:FragmentInputIndexArgument];

            [icbRenderCommand drawPrimitives:MTLPrimitiveTypeTriangle vertexStart:0 vertexCount:36 instanceCount:1 baseInstance:i];
        }
    }
...
- (void)drawInMTKView:(nonnull MTKView *)view
{
    ...
    [renderEncoder useResource:_verticesBuffer usage:MTLResourceUsageRead];
    [renderEncoder useResource:_uniformBuffer usage:MTLResourceUsageRead];
    [renderEncoder useResource:_indirectObjectParamsBuffer usage:MTLResourceUsageRead];
    [renderEncoder useResource:_fragmentArgumentBuffer usage:MTLResourceUsageRead];
    ...
    [renderEncoder executeCommandsInBuffer:_indirectCommandBuffer withRange:NSMakeRange(0, _cubeNum)];
